...
def joint_callback(data):
    ...
    '''
    if posser[0] > 2.3: 
        posser[0] = posser[0] - 0.5
    elif posser[0] < -2.3:
        posser[0] = posser[0] + 0.5
    
    if posser[1] > 2.3: 
        posser[1] = posser[1] - 0.5
    elif posser[1] < -2.3:
        posser[1] = posser[1] + 0.5
    
    if posser[2] > 2.3: 
        posser[2] = posser[2] - 0.5
    elif posser[2] < -2.3:
        posser[2] = posser[2] + 0.5

    if posser[3] > 2.3: 
        posser[3] = posser[3] - 0.5
    elif posser[3] < -2.3:
        posser[3] = posser[3] + 0.5

    if posser[4] > 2.3: 
        posser[4] = posser[4] - 0.5
    elif posser[4] < -2.3:
        posser[4] = posser[4] + 0.5

    if posser[5] > 2.3: 
        posser[5] = posser[5] - 0.5
    elif posser[5] < -2.3:
        posser[5] = posser[5] + 0.5
    '''
    '''
    elif posser[1] > 2.3 or posser[1] < -2.3:
        posser[1] = 0.0
    elif posser[2] > 2.3 or posser[2] < -2.3:
        posser[2] = 0.0
    elif posser[3] > 2.3 or posser[3] < -2.3:
        posser[3] = 0.0
    elif posser[4] > 2.3 or posser[4] < -2.3:
        posser[4] = 0.0
    elif posser[5] > 2.3 or posser[5] < -2.3:
        posser[5] = 0.0

    '''
    #assign_joint_value(posser[0], posser[1], posser[2], posser[3], posser[4], posser[5])
    # check_publishers_connection(pub)
...
def relative_joint_callback(data):
    ...
    relative_joint_value(pos[0], pos[1], pos[2], pos[3], pos[4], pos[5])
    # check_publishers_connection(pub)
    complete_msg.data = True
    pub.publish(complete_msg)
